  ...
  <div class="page">
    ...
    <div class="weui-flex">
      <div class="weui-flex__item">
        ...
        <div class="right f14 card rslineh">发帖卡:<span>232</span>张</div>
      </div>
    </div>
    <div class="weui-flex">
      <div class="weui-flex__item">
    ...
    <template v-for="item in article">
      <div class="weui-flex">
        <div class="weui-flex__item">
          ...
          <div class="cleft">
            <div>{{item.Title}}</div>
            <div class="num"><span>{{item.Author}}</span><span>{{getDateDiff(item.ModifyTime)}}</span><span>{{item.ReadNum}}</span> </div>
          </div>
        </div>
      </div>
    </template>
...
export default {
  ...
  methods: {
    ...
    initArticle: function () {
      get("/Article_Record/GetArticle?Page=1&PageSize=20", {}).then(res => {
        if (res.Code == '200') {
